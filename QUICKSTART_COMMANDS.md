...
export TASK3=copilot-docs
export BR1=feat/$TASK1
export BR2=feat/$TASK2
export BR3=feat/$TASK3
```
...
git worktree add -b "$BR3" "$WT_ROOT/$TASK3" "$BASE"
...
```bash
cd "$WT_ROOT/$TASK3"
# start GitHub Copilot for task 3
```
...
git log --oneline --decorate "$BASE..$BR3"

git diff --stat "$BASE..$BR1"
git diff --stat "$BASE..$BR2"
git diff --stat "$BASE..$BR3"
```
...
git worktree remove "$WT_ROOT/$TASK3"
git branch -D "$BR3"
git push origin --delete "$BR3"   # only if pushed before
```
